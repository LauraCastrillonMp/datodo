...
export class DataStructuresService {
  ...
  async findOne(id: number) {
    ...
    if (!ds) throw new NotFoundException(`Data structure with ID ${id} not found`);
    return ds;
  }

  async update(id: number, dto: UpdateDataStructureDto, userId: number, userRole: UserRole) {
    const ds = await this.findOne(id);
    if (userRole !== UserRole.admin && ds.createdBy !== userId) {
      throw new ForbiddenException('You do not have permission to update this data structure');
    }
  ...
  async remove(id: number, userId: number, userRole: UserRole) {
    const ds = await this.findOne(id);
    if (userRole !== UserRole.admin && ds.createdBy !== userId) {
      throw new ForbiddenException('You do not have permission to delete this data structure');
    }
  ...
  async findBySlug(slug: string): Promise<DataStructure & {
    creator: {
      id: number;
      name: string;
      username: string;
    };
    contents: Prisma.DataStructureContentGetPayload<{}>[];
  }> {
    ...
    if (!dataStructure) {
      throw new NotFoundException(`Data structure with slug '${slug}' not found`);
    }
