...
export function QuizComponent({
  ...
  useEffect(() => {
    const loadQuizResults = async () => {
      if (selectedQuizId && quizzes.length > 0) {
        ...
        if (selectedQuiz) {
          const res = await apiClient.getQuizResults(
            Number(dataStructureId),
            selectedQuizId
          );
          if (
            res &&
            ...
            const sortedByScore = [...res.data].sort(
    ...
    if (selectedAnswer !== null) {
      ...
      for (let i = 0; i <= currentQuestion; i++) {
        if (
          ...
          quiz!.questions[i].correctAnswers.includes(updatedAnswers[i]!)
        ) {
          newScore++;
        }
      }
  ...
  const finishQuiz = async (finalAnswers?: (number | null)[]) => {
    ...
    const answeredQuestions = answersToUse.filter(
      (answer) => answer !== null
    ).length;
    if (answeredQuestions === 0) {
      setSubmitError(
        "Por favor, responde al menos una pregunta antes de enviar."
      );
      return;
    }
    ...
    console.log("Submitting quiz with answers:", {
      ...
      quizQuestions: quiz?.questions.map((q, idx) => ({
        ...
        selectedOptionId:
          answersToUse[idx] !== null
            ? quiz.questions[idx].options[answersToUse[idx]]?.id
            : null,
      })),
    });
    ...
    try {
      if (quiz) {
        ...
        const answers = quiz.questions.map((q, idx) => {
          ...
          if (
            ...
            selectedAnswerIndex >= quiz.questions[idx].options.length
          ) {
            return {
              questionId: q.id,
              selectedOptionIds: [],
          ...
          const selectedOptionIds = [
            quiz.questions[idx].options[selectedAnswerIndex].id,
          ];
      ...
      setSubmitError(
        err.message || "Error saving your result. Please try again."
      );
      ...
      <Card>
        <CardContent className="text-center py-8">
          <p className="text-muted-foreground mb-4">
            No hay cuestionario disponible para esta estructura de datos.
          </p>
          {(recentResult !== null || bestResult !== null) && (
            <div className="mb-4 space-y-1">
              {recentResult !== null && (
                <div>
                  Tu puntuación más reciente:{" "}
                  <span className="font-semibold">{recentResult}%</span>
                </div>
              )}
              {bestResult !== null && (
                <div>
                  Tu mejor puntuación:{" "}
                  <span className="font-semibold">{bestResult}%</span>
                </div>
              )}
            </div>
          )}
          <p className="text-sm text-muted-foreground">
            Por favor, contacta a un administrador para crear un cuestionario
        ...
        <CardContent className="space-y-4">
          {timeRanOut ? (
            <div className="text-center">
              ...
              <p className="text-muted-foreground mb-4">
                Respondiste correctamente {score} de {quiz.questions.length}{" "}
                preguntas
              </p>
              <p className="text-sm text-muted-foreground">
                Tu puntuación:{" "}
                <span className="font-semibold">{backendScore}%</span>
              </p>
            </div>
          ) : (
            <div className="text-center">
    ...
    <Card>
      <CardHeader>
        ...
        <div className="space-y-2">
          ...
          <Progress
            value={quiz ? (currentQuestion / quiz.questions.length) * 100 : 0}
          />
        </div>
      </CardHeader>
