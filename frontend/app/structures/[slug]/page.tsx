...
export default function DataStructurePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const getStructureType = () => {
    ...
    if (slug.includes('linked') || slug.includes('enlazada')) return 'linked-list'
    if (slug.includes('tree') || slug.includes('arbol') || slug.includes('bst')) return 'tree'
  ...
  const needsInputValue = (operationName: string) => {
    const operation = operationName.toLowerCase()
    return operation.includes('insert') || operation.includes('add') || operation.includes('push') || 
           operation.includes('enqueue') || operation.includes('append') || operation.includes('set')
  }

  const needsSearchValue = (operationName: string) => {
    const operation = operationName.toLowerCase()
    return operation.includes('search') || operation.includes('find') || operation.includes('lookup') || 
           operation.includes('get') || operation.includes('contains')
  }
    ...
    return (
      <div className="p-6">
        <div className="text-center space-y-4">
          <h1 className="text-2xl font-bold text-red-600">Error al Cargar Contenido</h1>
  ...
  if (!dataStructure) {
    return (
      <div className="p-6">
        <div className="text-center space-y-4">
          <h1 className="text-2xl font-bold text-red-600">Estructura de Datos No Encontrada</h1>
          <p className="text-muted-foreground">La estructura de datos solicitada no pudo ser encontrada.</p>
          <Button onClick={() => router.push('/')}>
            <ArrowRight className="w-4 h-4 mr-2" />
            Volver a Estructuras
          </Button>
